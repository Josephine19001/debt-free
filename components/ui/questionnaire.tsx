  ...
  const renderOption = (option: any, stepKey: string) => {
    ...
    return (
      <Pressable
        ...
        className={`p-6 rounded-2xl mb-4 border border-pink-100`}
        style={{
      ...
      >
        {option.icon && (
          <View className="flex-row items-center mb-2">
            ...
            <Text className={`text-xl font-semibold ${isSelected ? 'text-white' : 'text-black'}`}>
              {option.label}
            </Text>
          </View>
        )}
        ...
        {!option.icon && (
          <Text
            className={`text-xl font-semibold mb-2 ${isSelected ? 'text-white' : 'text-black'}`}
          >
            {option.label}
          </Text>
        )}
        ...
        <Text
          className={`${isSelected ? 'text-white' : 'text-gray-600'} ${option.icon ? '' : 'ml-0'}`}
        >
          {option.description}
        </Text>
      </Pressable>
        ...
        <View className="px-4 py-4">
          <View className="flex-row items-center mb-4">
            ...
            <View className="flex-1">
              <View className="bg-gray-200 rounded-full h-1">
                <View
        ...
        <ScrollView className="flex-1 px-4">
          {isComplete || isGenerating ? (
            <View className="flex-1 justify-center items-center">
              ...
              <Text className="text-3xl font-bold text-center mb-4">Generating your goals...</Text>
            ...
            <View>
              <Text className="text-3xl font-bold mb-2">{currentStep.title}</Text>
              <Text className="text-gray-600 text-lg mb-8">{currentStep.subtitle}</Text>
